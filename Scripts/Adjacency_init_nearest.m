...
tic;
Ass_matrix_extend = zeros(M,N+G*M);
Ass_matrix_extend(:,1:N) = Ass_matrix;
Ass_matrix_extend(1,N+1:N+G) = 1;
Ass_matrix_extend(2,N+G+1:N+2*G) = 1;
Ass_matrix_extend(3,N+2*G+1:N+3*G) = 1;
Ass_matrix_extend(4,N+3*G+1:N+G*4) = 1;
for p=1:(N + G*M)
   for q=1:(N + G*M)
        BS_p = find(Ass_matrix_extend(:,p));  % BS index of user p
        BS_q = find(Ass_matrix_extend(:,q));  % BS index of user q
        if p<=N && q<=N  % Actual user -> Actual user
            ...
            if Group_q == Group_p || BS_p ~= BS_q  % user p and user q are in the same sub-channel
                continue;
            else
        ...
        elseif p>N && q>N || BS_p ~= BS_q    % Virtul user p -> Virtul user q
            continue;
        end
